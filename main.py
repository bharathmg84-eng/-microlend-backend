...
_MODEL_CANDIDATES = [
    os.path.join(BASE_DIR, "microlend_model.pkl"),          # same folder as main.py
    os.path.join(BASE_DIR, "model", "microlend_model.pkl"), # legacy model/ subfolder
]
...
# ── DB SETUP ──────────────────────────────────────────────────────────────────
def get_db():
    conn = sqlite3.connect(DB_PATH, timeout=10)   # wait up to 10 seconds if locked
...
def compute_score(req: ApplicationRequest) -> dict:
    ...
    # SHAP is temporarily disabled to prevent OOM on Render free tier
    # shap_vals = explainer.shap_values(features)[0]
    # factors = build_shap_factors(shap_vals)
    # Dummy factors so the UI doesn't break:
    factors = [
        {"label": "Business Vintage", "score": 70, "weight": "positive"},
        {"label": "Daily UPI Sales", "score": 80, "weight": "positive"},
        {"label": "Utility Bill History", "score": 85, "weight": "positive"},
        {"label": "Supplier Repayment", "score": 75, "weight": "positive"},
        {"label": "Loan-to-Revenue Ratio", "score": 65, "weight": "neutral"},
    ]
